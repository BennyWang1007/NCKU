...
void heapDecreaseKey(Heap *heap, int id, int dist, int *id_to_idx)
{
    ...
    while (idx > 0) {
        int parent = (idx - 1) / 2;
        if (heap->nodes[parent].dist <= node.dist)
            break;
...
bool checkHeap(Heap *heap)
{
    for (int i = 0; i < heap->size; i++) {
        ...
        if (right < heap->size &&
            heap->nodes[i].dist > heap->nodes[right].dist) {
            return false;
        }
    }
    return true;
...
int main(int argc, char **argv)
{
    ...
    MoveCost *localMoveCosts =
        (MoveCost *) malloc(sizeof(MoveCost) * pathCount);
    for (int i = 0; i < pathCount; i++) {
        if (sparsedMoveCost[i].dest >= localStartId &&
            sparsedMoveCost[i].dest < localEndId) {
            localMoveCosts[localPathCount] = sparsedMoveCost[i];
            localPathCount++;
        }
    }
    qsort(localMoveCosts, localPathCount, sizeof(MoveCost),
          compareMoveCostFrom);
    ...
    for (int i = 0; i < n; i++) {
        ...
        MPI_Allreduce(MPI_IN_PLACE, &distTableFrom0[globalMinId2], 1, MPI_INT,
                      MPI_MIN, comm);

        relex(distTableFrom0, localMoveCosts, moveCostStartIdx, globalMinId2, n,
              localPathCount, &heap, id_to_idx);
    }
