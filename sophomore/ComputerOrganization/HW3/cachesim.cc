...
cache_sim_t::cache_sim_t(size_t _sets,
                         size_t _ways,
                         size_t _linesz,
                         const char *_name)
...
cache_sim_t *cache_sim_t::construct(const char *config, const char *name)
{
    ...
    // MOD
    size_t *sets_idx = (size_t *) calloc(sets, sizeof(size_t));
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
    ...
    // MOD
    size_t idx = (addr >> idx_shift) & (sets - 1);
    size_t way = sets_idx[idx];
    sets_idx[idx]++;
    if (sets_idx[idx] >= ways)
        sets_idx[idx] -= ways;
    uint64_t victim = tags[idx * ways + way];
    tags[idx * ways + way] = (addr >> idx_shift) | VALID;
...
void cache_sim_t::clean_invalidate(uint64_t addr,
                                   size_t bytes,
                                   bool clean,
                                   bool inval)
{
