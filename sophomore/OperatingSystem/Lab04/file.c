 ...
 */
static ssize_t osfs_read(struct file *filp,
                         char __user *buf,
                         size_t len,
                         loff_t *ppos)
{
    ...
    // If the file has not been allocated a data block, it indicates the file is
    // empty
    ...
    // data_block = sb_info->data_blocks + osfs_inode->i_block * BLOCK_SIZE +
    // *ppos; if (copy_to_user(buf, data_block, len))
    //     return -EFAULT;
    ...
    data_block = sb_info->data_blocks +
                 osfs_inode->i_blocks_ptr[idx_of_iblock] * BLOCK_SIZE +
                 idx_of_block * BLOCK_SIZE + offset_in_block;
 ...
 */
static ssize_t osfs_write(struct file *filp,
                          ...
                          loff_t *ppos)
{
    ...
    int32_t block_needed =
        (*ppos + len) / BLOCK_SIZE + ((*ppos + len) % BLOCK_SIZE == 0 ? 0 : 1);

    if (osfs_inode->i_blocks < block_needed) {
        if (osfs_inode->i_blocks == 0) {
            // alloc data blocks
            osfs_inode->i_blocks_ptr = (uint32_t *) kmalloc(
                sizeof(uint32_t) * block_needed, GFP_KERNEL);
            osfs_inode->i_blocks_length = (int32_t *) kmalloc(
                sizeof(uint32_t) * block_needed, GFP_KERNEL);
            ...
            osfs_inode->i_blocks_ptr = (uint32_t *) krealloc(
                osfs_inode->i_blocks_ptr, sizeof(uint32_t) * block_needed,
                GFP_KERNEL);
            osfs_inode->i_blocks_length = (int32_t *) krealloc(
                osfs_inode->i_blocks_length, sizeof(uint32_t) * block_needed,
                GFP_KERNEL);
        ...
        if (osfs_realloc_multiple_data_blocks(sb_info, osfs_inode->i_blocks_ptr,
                                              osfs_inode->i_blocks_length,
                                              block_needed) < 0) {
            return -ENOSPC;
        }
    ...
    // pass, no need to limit the write length
    ...
    while (len > 0) {
        ...
        data_block = sb_info->data_blocks +
                     osfs_inode->i_blocks_ptr[idx_of_iblock] * BLOCK_SIZE +
                     idx_of_block * BLOCK_SIZE + offset_in_block;
        int32_t write_len = len > BLOCK_SIZE - offset_in_block
...
const struct file_operations osfs_file_operations = {
    .open =
        generic_file_open,  // Use generic open or implement osfs_open if needed
    .read = osfs_read,
    .write = osfs_write,
