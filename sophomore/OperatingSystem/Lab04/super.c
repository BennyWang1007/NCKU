 ...
 * Description: Initializes the superblock with filesystem-specific information
 * during mount. Inputs:
...
int osfs_fill_super(struct super_block *sb, void *data, int silent)
{
    ...
    root_osfs_inode->__i_atime = root_osfs_inode->__i_mtime =
        root_osfs_inode->__i_ctime = current_time(root_inode);
