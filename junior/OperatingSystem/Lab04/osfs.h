...
#define DATA_BLOCK_COUNT 200  // Assume there are 20 data blocks
...
struct osfs_sb_info {
    ...
    uint32_t first_level_index_block;  // First level block
};
...
struct osfs_inode {
    ...
    // uint32_t i_block;                   // Simplified handling, single data
    // block pointer
    uint32_t i_block;  // Simplified handling, single data block pointer for dir
    uint32_t *i_blocks_ptr;  // MODED
    int32_t *i_blocks_length;
};
...
// MODED
int osfs_alloc_multiple_data_blocks(struct osfs_sb_info *sb_info,
                                    uint32_t *block_nos,
                                    int32_t *block_length,
                                    int block_needed);
int osfs_realloc_multiple_data_blocks(struct osfs_sb_info *sb_info,
                                      uint32_t *block_nos,
                                      int32_t *block_length,
                                      int block_needed);
