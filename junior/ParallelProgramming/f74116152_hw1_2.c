                           ...
                           int rank)
{
    ...
    if (rank == 0) {
        ...
        for (int i = size - 1; i >= 0; --i) {
            while (result > 1 && cross(resultHull[result - 2],
                                       resultHull[result - 1], p[i]) <= 0) {
                --result;
            }
...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0) {
        ...
        for (int _rank = 1; _rank < size; ++_rank) {
            MPI_Recv(&receivedSize, 1, MPI_INT, _rank, 0, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
            Point *receivedHull = malloc(receivedSize * sizeof(Point));
            MPI_Recv(receivedHull, receivedSize, MPI_POINT, _rank, 0,
