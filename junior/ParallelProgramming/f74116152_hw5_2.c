...
void Andrew_monotone_chain(Point *p,
                           size_t size,
                           Point *resultHull,
                           int *resultSize)
{
    ...
    for (int i = size - 1; i >= 0; --i) {
        while (upper > 1 &&
               cross(upperHull[upper - 2], upperHull[upper - 1], p[i]) <= 0) {
            --upper;
        }
        upperHull[upper] = p[i];
        ++upper;
    ...
    for (int i = 0; i < size; ++i) {
        while (lower > 1 &&
               cross(lowerHull[lower - 2], lowerHull[lower - 1], p[i]) <= 0) {
            --lower;
        }
        lowerHull[lower] = p[i];
        ++lower;
                  ...
                  double min)
{
    if (curIdx == pointSize) {
        ...
        double sum = prim_mod(edges, edgeSize, nodes, pointSize, resultMST,
                              MSTSize, inMST);
        if (sum < min)
            return sum;
        return min;
    ...
    inMST[curIdx] = true;
    min = bruteForce(inMST, nodes, pointSize, curIdx + 1, hullIdx, hullSize,
                     edges, edgeSize, resultMST, MSTSize, min);
    for (int i = 0; i < hullSize; ++i) {
        if (hullIdx[i] == curIdx)
            return min;
    }
...
#pragma omp for
        for (int i = 0; i < work_count; ++i) {
            ...
            double my_min =
                bruteForce(inMSTs[i], local_nodes, n, start_idx, hullIdx,
                           hullSize, edges, edgeSize, resultMST, &MSTSize, min);
#pragma omp critical
            {
                if (my_min < min) {
                    min = my_min;
